...
/** What `/api/search?page=nope` and the named set in `/api/reports` produce. */
const BAD_PAGE = {
  source: 'query',
  message: 'page must be a whole number',
  // Never `['pagination', 'page']`: namespacing is output-only, so the set that
  // rejected is not named in the path even when a named set is what rejected.
  path: ['page'],
}
...
export function theValidationWire(nitroExtras: NitroExtras): void {
  ...
  it('delivers a composed route’s named and unnamed sets as one value', async () => {
    // Three fragments declare `query` and all three ran: the named set's output
    // nested under its name, the unnamed set's and the inline one's flat.
    expect(await $fetch('/api/reports?page=2&sort=desc&report=weekly')).toEqual(
      { report: 'weekly', page: 2, sort: 'desc' }
    )
  })

  it('never names the set that rejected in an issue’s path', async () => {
