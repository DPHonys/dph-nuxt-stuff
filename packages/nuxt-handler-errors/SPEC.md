...
**Status:** complete design, ready for a separate implementation effort. Nothing here is a
...
2. **On the wire, a declared failure is an ordinary HTTP error carrying one reserved key.**
   `data.__declaredError__ = { tag, status, ...payload }`. Its presence *is* the evidence that the
...
| Term | Meaning |
| --- | --- |
| **Variant** | One declared failure: `{ tag, status, ...payload }`. Tag is the discriminant; status never participates in narrowing. |
...
- **No writable explicit type parameter.** `Response` is declared with **no default type
  parameter**, so supplying any explicit type argument is `TS2558: Expected 2-3 type arguments, but
  got 1` — not a silent collapse to `any`. This is strictly stronger than 01a's original "never
  expose a leading explicit type parameter" mandate, and **replaces** it: the trap is
  unrepresentable, not merely discouraged (07 §1, `neg/n4`).
...
- **No `useAsyncData` counterpart.** `useAsyncData(key, handler)` takes a **function, not a route
  ...
  ```ts
  useAsyncData(() => $typedFetch('/api/users/1'))                              // compose with §3.5
...
interface $TypedFetch<DefaultT = unknown, DefaultR extends NitroFetchRequest = NitroFetchRequest>
  ...
  safe   <T = DefaultT, R extends NitroFetchRequest = DefaultR, O extends NitroFetchOptions<R> = NitroFetchOptions<R>>
         (request: R, opts?: O)
         : Promise<TypedResult<TypedInternalResponse<R, T, MethodOf<R, O>>,
                               Flatten<DeclaredErrorsOf<R, MethodOf<R, O>>>>>
  raw    <…>(request: R, opts?: O): Promise<FetchResponse<TypedInternalResponse<…>>>
  create <T = DefaultT, R extends NitroFetchRequest = DefaultR>(defaults: FetchOptions): $TypedFetch<T, R>
}

declare const $typedFetch: $TypedFetch

type TypedResult<T, D extends AnyVariant> = [D] extends [never]
  ? { ok: true, data: T }
  : { ok: true, data: T } | { ok: false, error: D }
```
...
export default defineTypedEventHandler(
  { errors: [orderErrors] },
  async (event, { fail }) => {
    const r = await event.$typedFetch.safe('/api/users/1')   // cookies + context forwarded
    if (!r.ok) {
...
- **Installed by a Nitro server plugin's `request` hook**, the line above Nitro's own, wrapping
  `event.$fetch` rather than the global (which is what preserves context):
...
export function declaredError<E extends AnyVariant>(
  err: NuxtError<DeclaredErrorBody<E>> | null | undefined
): E | undefined
export function declaredError(err: unknown): AnyVariant | undefined

// reactive sibling — templates
export function useDeclaredError<E extends AnyVariant>(
  err: Ref<NuxtError<DeclaredErrorBody<E>> | undefined>
...
### 3.8 Header handling — two surfaces, two different merges, and one shared helper would be a defect
...
```ts
addTypeTemplate({ filename: 'types/<name>.d.ts', getContents }, { nitro: true })
...
| Event | Measured |
| --- | --- |
| route added → our map contains it | 950 ms |
| route added → `nitro-routes.d.ts` contains it | 976 ms (**ours ~26 ms ahead**) |
| route removed → our map drops it | 631 ms |
| catalogue variant added (content edit) | emitted map **byte-identical** |

40 samples inside the convergence window: `agree 31, oursAhead 9, nitroAhead 0, misattributed 0`.

**Every disagreement was one-directional, in the safe direction**, exactly as the `:1441`-before-`:1442`
ordering predicts. A key we hold and Nitro does not is *unreachable*, because `MatchedRoutes` derives
its key universe from `keyof InternalApi`. Zero samples ever keyed a route to a handler file that was
not its own — the only shape that would produce a *wrong* error type rather than an absent one.
...
createError({
  ...
  statusMessage: tag,
  message: tag,
  data: { [DECLARED_ERROR_KEY]: { tag, status: variant.status, ...payload } },
})
```
...
| Path | Result |
| --- | --- |
| Client fetch | `err.data.data.__declaredError__` deep-equals `{tag, status, ...payload}` |
...
package drags them too. `Flatten<DeclaredErrorsOf<…>>` ships the same way through `.safe`.
...
hover and never expands. Naming the element type renders `SerializeObject<ValidationIssue>[]` at 93
chars with a **clickable name**; the fully-inline spelling renders 149 chars of expanded noise inside
the *same* unevaluated wrapper. Rendering only — the type is structurally exact either way. This binds
every future payload with an array field, not only §3.3's `issues`.

**Where legibility lands, judged (10 §4):**

| What the user hovers | Rendered | Verdict |
| --- | --- | --- |
| the composable | `UseTypedFetch` — 55 chars | ✅ vanilla is 94 |
...
| 3 | consumption-side positives | the **real playground**, against actual generated `.nuxt/types/*.d.ts` | `vue-tsc` in `typecheck` | already the `typecheck` script | **`types:extend` silently ceasing to fire** — the single most likely thing a Nuxt/Nitro patch breaks |
| 4 | the wire | playground e2e, `@nuxt/test-utils/e2e` | `vitest run` | needs `build` | that the envelope actually crosses the wire |

**Layer 3 is non-negotiable**, and is why hermetic-only was rejected. **Layer 4 stays at the
prototype's size** — six e2e tests over the three transport paths plus a route outside `/api/**` —
because its job is to prove the envelope crosses the wire, not to re-prove types. Layers 1 and 2 carry
the great majority of the assertions.
...
```ts
const BUDGETS = [
  { name: 'useTypedFetch',  max: 150 }, // measured  55 good / 1075 bad
  { name: 'H_error',        max: 260 }, // Flatten present / Serialize residue
  { name: 'H_invalidInput', max: 120 }, // measured  93 named / 149 inline
]
```
...
- **Ticket 20 — teach knip about `packages/*`** is open and **blocked on this spec**. `pnpm knip` is
  red (4 unused files, 3 unused deps) because knip cannot see that `module.ts` is a build entry, that
  `plugin.ts` reaches the app via `addPlugin`, or that fixtures are mounted through `rootDir`. Correct
  entry points *are* §3's export map. `pnpm check` is **not** blocked by this — knip is a separate
  script.
- **The `compatibility` change (§7.1)** is a one-line edit in two files and is recorded here rather
  than applied, per the map's destination.
- **The export map (§3, §8.2)** — `exports` **and** `typesVersions` — is the implementation effort's
  first packaging task, and unblocks ticket 20.
