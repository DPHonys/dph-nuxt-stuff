...
/**
 * Warns when the project sets a custom `nitro.errorHandler`. Call at setup,
 * before Nuxt fills the empty slot with its own handler, so the entries seen
 * are exactly the consumer's. `name` prefixes the warning.
 */
