 ...
 * text must be a function of routes and handler **paths**, never definition
...
function renderFile(routeBlocks: readonly string[]): string {
  return [
    ...
    // `KnownApiErrors` rather than a fresh global one. Nitro does exactly this.
    'export {}',
    '',
  ].join('\n')
}
 ...
 * and `Serialize` carries a second load: a handler type with an index
 * signature can reach `KnownErrorsOfHandler` as `unknown`, and
 * `Serialize<unknown>` is `never`, which closes that door for everything the
 * map can reach.
 */
