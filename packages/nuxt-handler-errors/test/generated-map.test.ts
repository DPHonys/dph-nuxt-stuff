...
import { COMPILERS } from './types/compilers'
import {
  assertHoverBudget,
  assertNoDiagnostics,
  createTypeHarness,
...
/** One built app: a tree plus its probe compiled by one compiler row. */
interface BuiltApp extends BuiltTree {
  readonly compilation: Compilation
}

/**
 * The prepared trees, keyed by name and built **once per process**.
 *
 * `nuxt prepare` knows nothing about compilers — the tree it leaves behind is
 * bytes on disk — so the compiler rows share it and pay only for their own
 * probe compilations. Without this the stock row would repeat every build the
 * bridge row already ran, and the builds are the expensive half.
 */
...
describe.each(COMPILERS)('on %s', (_label, compiler, dialect) => {
  let app: BuiltApp

  beforeAll(() => {
    // The tree builds once; what this row pays for is its own probe compile,
    // which under stock is the most expensive program in the repo.
    app = probeApp(ensureBuiltTree('base'), compiler, dialect)
  }, 600_000)
  ...
  describe('the generated map, in a real Nuxt app', () => {
    ...
    it('renders the composable’s error ref flat, in SPEC.md §9.3’s budget', () => {
      ...
      // the measurement back on 212 under both compilers. **The length is the
  ...
  describe('event.$typedFetch, in the real server program', () => {
    ...
    beforeAll(() => {
      server = probeServerApp(app.root, compiler, dialect)
    }, 600_000)
  ...
  describe('the lookup, over a map whose specifiers resolve to nothing', () => {
    ...
    beforeAll(() => {
      broken = probeApp(
        ensureBrokenFork(app, 'broken-specifiers'),
        compiler,
        dialect
      )
    }, 600_000)
