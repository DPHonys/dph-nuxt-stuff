...
import type { EmitMapSlot, NitroPathOptions } from '../../src/emit-map'
...
/** A second augmentation target, as the umbrella adds beside the errors map. */
const INPUTS_SLOT: EmitMapSlot = {
  interfaceName: 'RequestInputs',
  specifier: '@dphonys/nuxt-typed-handler/types',
  imports: [
    { names: ['Simplify'], from: 'nitropack/types' },
    { names: ['InputsOfHandler'], from: '@dphonys/nuxt-typed-handler/types' },
  ],
  extract: (handlerType) => `Simplify<InputsOfHandler<${handlerType}>>`,
}

const BOOM: NitroEventHandler = {
  route: '/api/boom',
  method: 'get',
  handler: '/app/server/api/boom.get.ts',
}
...
describe('slots', () => {
  it('defaults to the known-errors slot alone, byte for byte', () => {
    expect(emitMap([BOOM], { nitroOptions: NITRO })).toBe(
      emitMap([BOOM], { nitroOptions: NITRO, slots: [KNOWN_ERRORS_SLOT] })
    )
  })

  it('renders a second slot as a second `declare module` block, after the first', () => {
    const emitted = emitMap([BOOM], {
      nitroOptions: NITRO,
      slots: [KNOWN_ERRORS_SLOT, INPUTS_SLOT],
    })
    ...
    expect(emitted).toContain(
      [
        ...
        '  }',
        '}',
        `declare module '@dphonys/nuxt-typed-handler/types' {`,
        '  interface RequestInputs {',
        `    '/api/boom': {`,
        `      'get': Simplify<InputsOfHandler<typeof import('../../server/api/boom.get').default>>`,
        '    }',
        '  }',
        '}',
        'export {}',
  ...
  it('merges imports from one module onto one sorted line', () => {
    // Both slots lean on `nitropack/types`; two import lines for it would be
    // legal TypeScript but not the one file the umbrella asked for.
    const emitted = emitMap([BOOM], {
      nitroOptions: NITRO,
      slots: [
        KNOWN_ERRORS_SLOT,
        {
          ...INPUTS_SLOT,
          imports: [
            { names: ['Simplify', 'EventHandlerRequest'] as const, from: 'h3' },
            { names: ['InputsOfHandler'], from: INPUTS_SLOT.specifier },
            { names: ['Simplify'], from: 'nitropack/types' },
          ],
        },
      ],
    ...
    expect(emitted.split('\n').slice(1, 5)).toEqual([
      `import type { Serialize, Simplify } from 'nitropack/types'`,
      `import type { KnownErrorsOfHandler } from '@dphonys/nuxt-handler-errors/types'`,
      `import type { EventHandlerRequest, Simplify } from 'h3'`,
      `import type { InputsOfHandler } from '@dphonys/nuxt-typed-handler/types'`,
    ])
    expect(emitted.match(/from 'nitropack\/types'/g)).toHaveLength(1)
  })

  it('renders both empty interfaces from `emptyMap`, and `EMPTY_MAP` is the default one', () => {
    expect(emptyMap({ slots: [KNOWN_ERRORS_SLOT, INPUTS_SLOT] })).toBe(
      [
        '// Generated by @dphonys/nuxt-handler-errors - do not edit',
        `import type { Serialize, Simplify } from 'nitropack/types'`,
        `import type { KnownErrorsOfHandler } from '@dphonys/nuxt-handler-errors/types'`,
    ...
    expect(EMPTY_MAP).toBe(emptyMap())
    expect(EMPTY_MAP).toBe(emitMap([], { nitroOptions: NITRO }))
  })

  it('puts `generatedBy` in the banner and nowhere else', () => {
    const [banner, ...rest] = emitMap([BOOM], {
      nitroOptions: NITRO,
      generatedBy: '@dphonys/nuxt-typed-handler',
    }).split('\n')
    const [defaultBanner, ...defaultRest] = emitMap([BOOM], {
      nitroOptions: NITRO,
    }).split('\n')

    expect(banner).toBe(
      '// Generated by @dphonys/nuxt-typed-handler - do not edit'
    )
    expect(defaultBanner).toBe(
      '// Generated by @dphonys/nuxt-handler-errors - do not edit'
    )
