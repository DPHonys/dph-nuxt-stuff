...
// hook alike. Every case is an `Error`, which is what Nitro hands the hook.
...
describe('recognizeKnownError', () => {
  ...
  it('carries the payload fields that ride inside the marker', () => {
    ...
    expect(
      recognizeKnownError(createError({ data: { [KNOWN_ERROR_KEY]: rich } }))
    ).toEqual(rich)
  })
  ...
  it.each([
    ...
    [
      'a marker with no tag',
      createError({ data: { [KNOWN_ERROR_KEY]: { status: 403 } } }),
    ],
    [
      'a marker with no status',
      createError({ data: { [KNOWN_ERROR_KEY]: { tag: 'x' } } }),
    ],
    [
      'a non-object marker',
      createError({ data: { [KNOWN_ERROR_KEY]: 'forbidden' } }),
    ],
    ['a null marker', createError({ data: { [KNOWN_ERROR_KEY]: null } })],
    ['a null data', createError({ data: null })],
    ['no data at all', createError({ statusCode: 500 })],
