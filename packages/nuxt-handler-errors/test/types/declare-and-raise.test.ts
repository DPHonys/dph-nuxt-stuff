...
describe.each(COMPILERS)('declaring and raising, on %s', (_label, compiler) => {
  ...
  describe('the declaration surface', () => {
    it('holds every positive claim, with zero diagnostics', () => {
      assertNoDiagnostics(harness.compileAlone('pos/declare-and-raise.ts'))
    })
    ...
    it('renders each callable as its interface name, not its signature', () => {
      const compilation = harness.compileAlone('pos/declare-and-raise.ts')
