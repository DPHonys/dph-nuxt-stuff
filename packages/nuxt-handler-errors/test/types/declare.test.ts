 ...
 * inside it was two thirds of the old measurement, all of it noise.
 */
...
describe('what must not compile', () => {
  ...
  it('rejects `.pick()` of an unknown tag, listing the real tags', () => {
    // The union's members arrive in declaration order — stock never sorts
    // them. All three real tags are named, which is the claim.
  ...
  it('rejects a payload field that JSON serialization would throw on', () => {
    ...
    assertDiagnostic(harness.compileAlone('neg/unserializable-payload.ts'), {
      code: 2344,
      message: 'Payload field does not survive JSON serialization: outstanding',
    })
  })
})
