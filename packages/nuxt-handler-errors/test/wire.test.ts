...
describe('the declared-failure wire format', async () => {
  ...
  it('carries a validation failure’s issues across the wire, losslessly', async () => {
    // SPEC.md §3.3 end to end, against a real zod schema on a real server. The
    // request is hostile in four ways at once — a missing field, a wrong type,
    // a bad value under a key containing a dot, and a query that will not
    // coerce — and the whole point is that **one** response reports all of it.
    const error = await rejectionOf('/api/signup?code=nope', {
      method: 'POST',
      body: { 'contact.email': 'not-an-email', tags: [{ label: 7 }] },
    })

    expect(error, 'a declared failure must reject, not resolve').toBeDefined()
    expect(error.statusCode).toBe(400)

    const marker = error.data.data[DECLARED_ERROR_KEY]

    expect(marker.tag).toBe('invalid-input')
    expect(marker.status).toBe(400)

    // Every declared location is validated, so `query` is reported beside
    // `body` rather than after a second round trip (SPEC.md §11.4's rejection
    // of fail-fast per source).
    expect(
      marker.issues.map((issue: any) => [issue.location, issue.path])
    ).toEqual([
      ['body', ['name']],
      // The key containing a dot, recovered exactly. `.join('.')` at the call
      // site gives `contact.email`; a dotted-string `path` could not have been
      // told apart from a nested `contact` object (SPEC.md §11.4).
      ['body', ['contact.email']],
      // A segment array through an array of objects: the index stays a number,
      // which is what keeps it distinguishable from a key that looks like one.
      ['body', ['tags', 0, 'label']],
      ['query', ['code']],
    ])

    // Machine-readable is the whole claim: h3 1.15.11 throws the raw exception
    // as `data`, which serializes to `{ name, message }` — a sentence, not a
    // field list.
    for (const issue of marker.issues)
      expect(issue.message).toBeTypeOf('string')
  })

  it('hands a valid request to the handler already parsed', async () => {
    // The other half of the same route: `code` arrives coerced to a number,
    // which is what SPEC.md §7.4 records h3 v2's own validated handler as being
    // unable to represent at all — its query output is coerced back to string
    // by construction.
    expect(
      await $fetch('/api/signup?code=42', {
        method: 'POST',
        body: {
          name: 'Ada',
