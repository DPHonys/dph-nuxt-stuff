...
The per-piece take / adapt / drop inventory of `nuxt-handler-errors`'s
internals (emitter, module wiring, wire, raise path, fetch runtime), consumed
by the implementation step. The requirements distilled from it — payload
schemas, channel gating, layering, observability — live in DESIGN.md §5; this
file holds the piece-level detail. Source: the old
`packages/nuxt-handler-errors/src/**` and its README limitations.
...
- **Derived from handler _paths_, never catalogue content** — the map says
...
- **`types/` placement is load-bearing** — an unresolved `import('…')` in a
  `.d.ts` produces _no diagnostic_; the entry silently becomes the error type.
...
- The extractor. Old: `ExtractErrorsSafe<typeof handler>` with
  ...
  DESIGN.md §4). Still wrapped in `Simplify<Serialize<…>>` — serialization
...
- Names: `TypedApiErrors` → `KnownApiErrors`; the emitted header comment; the
  ...
  takes back the old identity (DESIGN.md §1, the naming pass).
...
- **The plugin split** — client-only app plugin + Nitro plugin for the global,
  a _separate_ Nitro plugin for the event surface — exists so each is
...
- **The custom `nitro.errorHandler` warning** — take, adapted to the chain
  seam: the module now prepends to the `errorHandler` array (DESIGN.md §5,
  channel gating), but a consumer handler that drops `data` still silently
  kills the package. Whether v1 wants a runtime smoke-check on top is §6.
- **`ModuleOptions = {}`** — keep empty. The observability recognizer and
  channel gating both need no option (DESIGN.md §5; gating is enabled by the
  token's presence in runtimeConfig).
...
- **DROP: `statusMessage: tag`.** The old package's documented reason-phrase
  leak was its own wire's fault (measured, DESIGN.md §3). Already a standing
...
- **The floor read (`readFloor`)** — take as the matcher's internal guard:
  ...
  _unknown_ (the direction every consumer handles). One nuance to carry into
  the implementation: **the marker sits at two depths** (DESIGN.md §3). The
  old reader only ever handled the fetched shape
  (`error.data.data.__knownError__`); the server-side recognizer must also
  handle the raise-site shape (`error.data.__knownError__`).

## 4. The raise path and catalogue runtime (`errors.ts`) — ADAPT: ideas, not code

The catalogue container is gone (variants as values, DESIGN.md §1), so the
...
- **A runtime brand the module instance owns, checked loud and early.** The
  ...
  two-copies-in-the-tree state at _module evaluation_, not on whichever
  ...
  must be runtime-real (DESIGN.md §4: an optional phantom on a value is a
  weak type). Same mechanism, new unit: `defineError` attaches the
  module-private marker to each `KnownError` value;
  `defineCheckedEventHandler` verifies every element of `errors: [...]` at
  declaration and throws the foreign-copy error naming the index. Take the
  error message's diagnosis text too — it was good.
...
- **First-wins tag resolution** is superseded: `ConflictGuard` makes divergent
  ...
  is already standing (DESIGN.md §1): **one error per distinct tag in
  `pick()` and in the handler's resolved list** — dedupe at build-the-list
  time, first occurrence wins.

## 5. The fetch runtime — TAKE the measurements, rewrite the semantics

The three header merges and their _deliberate unsharing_ are the most
...
- **`accept: application/json` on every request** — without it Nitro's
  ...
  `baseURL`, and a declared 403 arrives as HTML. (Re-verified in DESIGN.md §3
  including why ofetch never rescues it.)
- **Global form:** merge through `Headers`, hand on _as_ `Headers` (ofetch's
...
The channel-gating token header (DESIGN.md §5) rides the same machinery, in
all three forms.
...
- **`create` returns the typed interface**; instance-header semantics as
  ...
  passed through — both settled in DESIGN.md §1.)
- **The event plugin's `request`-hook placement** (fifth closure beside
  ...
  **thunk + first-call `EventFetchUnavailableError` guard** naming the
  `@experimental` skew (DESIGN.md §3 says the guard stays right).
...
Implementation deltas from the old code (all locked in DESIGN.md §1):
...
- New runtime with no old counterpart: `useCheckedAsyncData`'s
  unwrap-or-rethrow (measured safe, DESIGN.md §3), `useRequestCheckedFetch`
  (three-line mirror of vanilla's), and `matchError` itself (floor read +
  arm dispatch + fallback; trivially small, and the _only_ read path —
  `useDeclaredError`/`declaredError` have no successor by design).

## 6. Open candidates, none blocking

- **A dev-time wire smoke check.** The custom-`errorHandler` warning fires on
  the override's _presence_, but a handler that keeps `data` is fine and one
...
- **Test infrastructure as an internal worth taking:** the old package's
  type-test harness with pos/neg fixture programs, and the
  mutation-visibility arguments baked into the plugin split. The harness
  approach carries over even though every fixture is rewritten.

---

## Standing requirements already locked elsewhere (collected, not new)

The implementation checklist DESIGN.md has already banked:
