 ...
 * is bytes on disk. Compiling that tree is `attach` below.
 *
    ...
    `${JSON.stringify(
      {
        ...
        compilerOptions: {
          paths: {
            '@dphonys/nuxt-handler-errors-old/types': [
              join(PACKAGE_ROOT, 'src/runtime/types/index.ts'),
            ],
            '@dphonys/nuxt-handler-errors-old/server': [
              join(PACKAGE_ROOT, 'src/runtime/server/index.ts'),
            ],
            '@dphonys/nuxt-handler-errors-old/shared': [
...
const CONSUMER_PRELUDE = [
  `import type { TypedApiErrors } from '@dphonys/nuxt-handler-errors-old/types'`,
