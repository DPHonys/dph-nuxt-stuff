...
/**
 * Layer 1 for `$typedFetch` and `$typedFetch.safe`.
 *
...
/**
 * The named-interface mandate, re-measured on this surface.
 *
 ...
 * flattering it.
 *
 * Set at ~2× the good value rather than the usual 1.5×, for the
...
describe('$typedFetch, over a hand-written map', () => {
  ...
  it('holds every claim made about the namespace', () => {
    ...
    // compiles clean. Nothing is filtered by file: it
  ...
  it('leaves the throwing form byte-identical to vanilla', () => {
    // The degradation lock on the default entry point, in the
  ...
  it('keeps a payload behind the tag, so the union is a real one', () => {
    ...
    // the shape floor or TypeScript's error type — either of which
    // would let this read through.
